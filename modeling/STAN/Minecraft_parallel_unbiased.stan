...
// Unbiased social learning model
...
real partial_sum(int[] y_slice, //important: uses only the slice from start to end
                   ...
                   int[] group,
                   int[] decision_number,
                   int[] all_visible_player_distance_NA,
...
for(i in start:end){
  ...
  if(all_visible_player_distance_NA[i]==0){
...
data{
   ...
   int all_visible_player_distance_NA[N];   //Whether others were observed for choice
...
int grainsize = 1;
 ...
 target += reduce_sum(partial_sum, choice , grainsize,
                     ...
                     all_visible_player_distance_NA,
