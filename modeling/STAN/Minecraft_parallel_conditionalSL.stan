...
for(i in start:end){
  ...
  if(successful_NA[i]==0 && unsuccessful_NA[i]==0){
    ...
    time_weights[4] = beta_t_ind[1, env[i]] + v_ID[id[i],N_feat+1] + v_group[group[i],N_feat+1];
    time_weights[5] = beta_t_ind[2, env[i]] + v_ID[id[i],N_feat+2] + v_group[group[i],N_feat+2];
...
parameters{
   ...
   matrix[2,2] beta_t_ind;

   // Varying effects clustered on individual
    matrix[N_feat+2,N_id] z_ID;
    vector<lower=0>[N_feat+2] sigma_ID;
    cholesky_factor_corr[N_feat+2] Rho_ID;

    // Varying effects clustered on groups
     matrix[N_feat+2,N_groups] z_group;
     vector<lower=0>[N_feat+2] sigma_group;
     cholesky_factor_corr[N_feat+2] Rho_group;
}
...
transformed parameters{
      matrix[N_id,N_feat+2] v_ID;
      matrix[N_groups,N_feat+2] v_group;
