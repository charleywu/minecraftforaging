...
//Conditional Social Learning (weight of social features depends on time since last personal success)
...
for(i in start:end){
  ...
  if(successful_NA[i]==0 && unsuccessful_NA[i]==0){
    ...
    time_weights[1] = beta_t_ind[1, env[i]] + v_ID[id[i],N_feat+1] + v_group[group[i],N_feat+1];
    time_weights[4] = beta_t_ind[2, env[i]] + v_ID[id[i],N_feat+2] + v_group[group[i],N_feat+2];
    time_weights[5] = beta_t_ind[3, env[i]] + v_ID[id[i],N_feat+3] + v_group[group[i],N_feat+3];
 ...
 if(successful_NA[i]==0 && unsuccessful_NA[i]==1){
  ...
  //Compute choice probabilities using softmax function
  p = softmax(features * ((weights[{1,2,3,4} , env[i]] + v_ID[id[i],{1,2,3,4}]' + v_group[group[i],{1,2,3,4}]' ) + time_weights*last_ind_reward[i] ) );
 }
...
if(successful_NA[i]==1 && unsuccessful_NA[i]==0){
    vector[4] time_weights = rep_vector(0,4);
    time_weights[1] = beta_t_ind[1, env[i]] + v_ID[id[i],N_feat+1] + v_group[group[i],N_feat+1];
    time_weights[4] = beta_t_ind[3, env[i]] + v_ID[id[i],N_feat+3] + v_group[group[i],N_feat+3];
    matrix[B[i] , 4] features = to_matrix(Feature_matrix[i,1:B[i],{1,2,3,5}]) ;
  //Compute choice probabilities using softmax function
  p = softmax(features * ((weights[{1,2,3,5} , env[i]] + v_ID[id[i],{1,2,3,5}]' + v_group[group[i],{1,2,3,5}]' )+ time_weights*last_ind_reward[i] ) );
}
...
if(successful_NA[i]==1 && unsuccessful_NA[i]==1){
  ...
  //Compute choice probabilities using softmax function
  p = softmax(features * ((weights[{1,2,3} , env[i]] + v_ID[id[i],{1,2,3}]' + v_group[group[i],{1,2,3}]' )+ time_weights*last_ind_reward[i] ) );
}
...
parameters{
   ...
   matrix[3,2] beta_t_ind;

   // Varying effects clustered on individual
    matrix[N_feat+3,N_id] z_ID;
    vector<lower=0>[N_feat+3] sigma_ID;
    cholesky_factor_corr[N_feat+3] Rho_ID;

    // Varying effects clustered on groups
     matrix[N_feat+3,N_groups] z_group;
     vector<lower=0>[N_feat+3] sigma_group;
     cholesky_factor_corr[N_feat+3] Rho_group;
}
...
transformed parameters{
      matrix[N_id,N_feat+3] v_ID;
      matrix[N_groups,N_feat+3] v_group;
...
int grainsize = 1;
  ...
  to_vector(beta_t_ind) ~ normal(0,3);
